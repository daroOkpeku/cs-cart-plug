...
function fn_delly_man_get_category_data_pre($category_id, $field_list, $get_main_pair, $skip_company_condition, $lang_code)
{
	...
	if (!empty($auth['user_id']) && AREA == 'C') {
		...
		$viewed_categories = db_get_field('SELECT categories FROM ?:delly_man WHERE user_id = ?i', $auth['user_id']);
		...
		$viewed_categories = serialize($viewed_categories);
		 ...
		 db_query('REPLACE INTO ?:delly_man VALUES (?i, ?s)', $auth['user_id'], $viewed_categories);
